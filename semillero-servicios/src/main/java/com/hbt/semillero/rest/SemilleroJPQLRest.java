...
import com.hbt.semillero.dto.EJERCICIO12DTO;
...
//			comicSuperman = this.consultarComicPorNombre(comicSuperman);
	...
	/**
	 * 
	 * Metodo encargado de traer todos los nombres de comics que se encuentren en la tabla comic y determina que nombres
	 * de los comics supera la longitud del valor lengthComic y cuales no 
	 * <b>Caso de Uso</b>
	 ...
	 */
	@GET
	@Path("/ejercicioClase12")
	@Produces(MediaType.APPLICATION_JSON)
	@SuppressWarnings("unchecked")
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public String consultarComicTamanioNombre (short lengthComic) {
		
		
		List<String> nombresDeComicsConLongitudMenoralengthComic=new ArrayList<String>(); //lista que contiene los nombres de los comics con nombre menor a lengthComic
		List<String> nombresDeComicsConLongitudMayoralengthComic=new ArrayList<String>(); //lista que contiene los nombres de los comics con nombre menor a lengthComic
		EJERCICIO12DTO resultado= new EJERCICIO12DTO();
	
		
		try {
			log.info("Comics procesados exitosamente");	
		String consulta = "SELECT SCNOMBRE FROM COMIC c";
		Query queryFindAllComic = em.createNativeQuery(consulta);
		List<Comic> listaComics = queryFindAllComic.getResultList();
		
		for (int i=0;i<listaComics.size();i++)
		{
			String NombreComic=listaComics.get(i).getNombre();
			if(NombreComic.length()<lengthComic)
			{
				nombresDeComicsConLongitudMenoralengthComic.add(NombreComic);
			}
			else
				nombresDeComicsConLongitudMayoralengthComic.add(NombreComic);
		}
		resultado.setNombreComicsMayorLongitud(nombresDeComicsConLongitudMayoralengthComic);
		resultado.setNombreComicsMenorLongitud(nombresDeComicsConLongitudMenoralengthComic);
